...
class NullHypothesisResidual(Residual):
    # Returns a constant cost with zero Jacobian.
    # If used in a Gaussian mixture factor, it plays the role
    # that a state/landmark combination can be associated to it
    # instead of a measurement.
    threshold: float
    err_dim: int

    def __init__(self, keys: List[Hashable], threshold: float, err_dim: int):
        super().__init__(keys)
        self.threshold = threshold
        self.err_dim = err_dim

    def sqrt_info_matrix(self, states: List[State]):
        return np.eye(self.err_dim)

    def evaluate(
        self, states: List[State], compute_jacobians: List[bool] = None
    ) -> Tuple[np.ndarray, List[np.ndarray]]:
        e = np.sqrt(self.threshold) * np.ones(self.err_dim)
        if compute_jacobians:
            jacobians = [None] * len(states)
            for lv1 in range(len(compute_jacobians)):
                if compute_jacobians[lv1]:
                    jacobians[lv1] = np.zeros((self.err_dim, states[lv1].dof))
            return e, jacobians
        return e
